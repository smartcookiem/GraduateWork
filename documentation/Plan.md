...
2. **Поле "Номер карты":**
   ...
   2.1. **Оплата картой с невалидным номером карты:**

    1. Открыть [страницу](https://localhost:8080/).
    2. Нажать кнопку "Купить" (оплата картой).
    3. В поле "Номер карты" **ввести не валидный номер карты** (4444 4444 4444 4442), остальные поля заполнить валидными данными..
    4. Нажать кнопку "Продолжить";
   ...
   2.2. **Оплата без заполненного поля "Номер карты":**
    ...
    1. Открыть [страницу](https://localhost:8080/).
    2. Нажать кнопку "Купить" (оплата картой).
    3. В поле "Номер карты" **ввести не полный номер карты** (4444 4444 4444 444), остальные поля заполнить валидными данными.
    4. Нажать кнопку "Продолжить";
...
3. **Поле "Месяц":**
   ...
   3.1. **Оплата картой без заполненного поля "Месяц":**
   ...
   3.2. **Оплата картой с вводом месяца нижней границы значения:**
    ...
    3. В поле "Месяц" **ввести не валидный месяц** (нижнее граничное значение 00), остальные поля заполнить валидными данными.  
   ...
   3.3. **Оплата картой с вводом текущий месяц -1, и текущий год:**
    ...
    3. В поле "Месяц" **ввести не валидный месяц** (текущий месяц -1, и текущий год), остальные поля заполнить валидными данными..  
   ...
   3.4. **Оплата картой с вводом месяца верхней границы значения:**
    ...
    3. В поле "Месяц" **ввести не валидный месяц** (верхнее граничное 13), остальные поля заполнить валидными данными.  
...
4. **Поле "Год":**
   ...
   4.1. **Оплата картой без заполненного поля "Год":**
   ...
   4.2. **Оплата картой с вводом предыдущего года:**
    ...
    3. в поле "Год" **ввести не валидный год** (текущий год - 1), остальные поля заполнить валидными данными.  
   ...
   4.3. **Оплата картой с вводом будущего года:**
    ...
    3. В поле "Год" **ввести не валидный год** (текущий год + 6), остальные поля заполнить валидными данными.  
...
5. **Поле "Владелец":**
   ...
   5.1. **Оплата картой без заполненного поля "Владелец":**
   ...
   5.3. **Оплата картой с вводом в поле "Владелец" спецсимволов:**
    ...
    3. В поле "Владелец" **ввести $#@** (любые спецсимволы), остальные поля заполнить 
валидными данными.
   ...
   5.4. **Оплата картой с вводом в поле "Владелец" двузначного числа:**
    ...
    3. В поле "Владелец" **ввести двузначное число ** (от 10 до 99), остальные поля заполнить валидными данными.  
...
6. **Поле "CVC/CVV":**
   ...
   6.1. **Оплата картой без заполненного поля "CVC/CVV":**
   ...
   6.2. **Оплата картой с вводом двузначного "CVC/CVV":**

    1. Открыть [страницу](https://localhost:8080/). 

    2. Нажать кнопку "Купить" (оплата картой).

    3. В поле "CVC/CVV" **ввести двузначное число**(10-99), остальные поля заполнить валидными данными.  
